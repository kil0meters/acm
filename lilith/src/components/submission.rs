...
pub fn SubmissionFeedback(props: &SubmissionFeedbackProps) -> Html {
    ...
    if let Some(error) = &submission.error {
        html! {
            <div class="bg-red-500 text-red-50 p-4 flex flex-col gap-2 h-full">
                <div class="flex items-start">
                    <h1 class="text-2xl font-bold">{ "error." }</h1>
                    if props.share {
                        <ShareButton
                            path={format!("/submissions/{}", submission.id)}
                            class="bg-red-700 hover:bg-red-600 text-red-50 rounded-full px-4 py-2 ml-auto text-sm transition-colors"
                        />
                    }
                </div>
        ...
        if submission.success {
            html! {
                <div class="flex-col flex p-4 bg-green-500 text-green-50 h-full">
                    <div class="flex items-start">
            ...
            html! {
                <div class="flex-col flex p-4 bg-white h-full">
                    <div class="flex items-start">
