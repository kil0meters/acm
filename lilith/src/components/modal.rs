        ...
        if props.shown {
            <div class="fixed left-0 right-0 top-0 bottom-0 bg-black/30 z-50">
                <div class="mt-[30vh] max-w-lg mx-auto p-2 relative">
                    ...
                    <button class="absolute top-4 right-4 rounded-full bg-slate-700 hover:bg-slate-500 text-slate-100 transition-colors px-4 py-2 text-sm" onclick={Callback::from(move |_| {
                        onclose.emit(());
                    })}>{ "Close" }</button>
                </div>
            </div>
